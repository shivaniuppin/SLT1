...
# Define a dictionary to map gestures to text
gesture_to_text = {
    "thumbs_up": "Good job!",
    "peace": "Peace",
    "okay": "Okay",
...
def classify_gesture(landmarks):
    ...
    # Detect "peace" (index and middle fingers up, others down)
    if index_tip < index_dip and middle_tip < middle_dip and ring_tip > middle_dip and pinky_tip > middle_dip:
        return "peace"

    # Detect "okay" (thumb and index finger form a circle, others extended)
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                cv2.putText(frame, f"Detected: {text}", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

                # Convert the gesture to speech
                if text != "Unknown Gesture":
                    engine.say(text)
                    engine.runAndWait()
